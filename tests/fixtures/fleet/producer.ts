...
//   {"notify": id, "text"}
//   {"clock": seconds}                        the registry's clock (starts at 0); fires the registry's
//                                             timers (decay) that are due by then
//   {"clockWhen": path, "clock": seconds}     the same once the file `path` appears, then reports
//                                             event "clock": it moves the clock without typing
...
export default function (pi: ExtensionAPI) {
  ...
  let now = 0;
  // The registry's timers run on its clock: a "clock" op fires the due ones.
  let seq = 0;
  const pending = new Map<number, { at: number; fn: () => void }>();
  const timers = {
    setTimeout: (fn: () => void, ms: number) => (pending.set(++seq, { at: now * 1000 + ms, fn }), seq),
  ...
  const tick = (seconds: number) => {
    ...
    for (const [id, t] of [...pending]) if (t.at <= now * 1000 && pending.delete(id)) t.fn();
  };
  ...
  pi.registerCommand("fx", {
    ...
    handler: async (args, ctx) => {
      ...
      for (const op of JSON.parse(args)) {
        ...
        else if ("prune" in op) registry.prune();
        else if ("clockWhen" in op) {
          const watcher = watch(dirname(op.clockWhen), () => {
            if (!existsSync(op.clockWhen)) return;
            watcher.close();
            tick(op.clock);
            appendFileSync(process.env.PI_HARNESS_EVENTS!, JSON.stringify({ event: "clock" }) + "\n");
          });
        } else if ("clock" in op) tick(op.clock);
      }
