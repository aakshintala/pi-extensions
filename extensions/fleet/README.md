...
- The viewer stays open when its item finishes, and closes once the item is removed. Its header keeps showing `esc back` even in that gap, so there is always an on-screen way out.
