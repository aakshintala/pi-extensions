...
- `bash({ command, timeout?, run_in_background? })` replaces Pi's built-in.
  ...
  - Ctrl+B moves every running foreground command to the background. Each
    call returns its job ID and log path. While a command runs in the
    foreground, its call shows a dim line carrying its elapsed time (#162),
    with `· ctrl+b to run in background` added unless Ctrl+B still moves the
    cursor left. The line needs the tool-display extension.
