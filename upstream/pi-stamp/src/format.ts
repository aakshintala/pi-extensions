...
// ponytail: cache ICU formatters; uncached, each render built ~1k fresh instances
...
function formatLocalized(
	...
	const formattedDate = cachedFormatter(`date|${locale ?? ""}|${timeZone ?? ""}`, () =>
		new Intl.DateTimeFormat(locale, {
			calendar: "gregory",
			timeZone,
			dateStyle: "medium",
...
function zonedParts(timestamp: number, timeZone: string | undefined): ZonedParts {
	const formatter = cachedFormatter(`zoned|${timeZone ?? ""}`, () =>
