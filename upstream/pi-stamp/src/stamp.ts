...
// ponytail: stamp entries are immutable, so recompute lines only when settings
// change; unmemoized, every TUI frame rebuilt every label (~1k ICU
// formatters/frame before the format.ts cache).
function settingsSignature(settings: Readonly<StampSettings>): string {
	return [
		settings.hourCycle,
		settings.showSeconds,
		settings.dateContext,
		settings.locale,
		settings.timeZone,
		settings.responseTiming,
		settings.assistantMetadata,
		settings.showExactTimeline,
...
function memoizeStampLines(
	...
	let lastLines: readonly RightAlignedLine[] = [];
	return () => {
		const signature = settingsSignature(getSettings());
...
function dynamicRightAlignedText(
	...
	let lastWidth = -1;
	let lastLines: readonly RightAlignedLine[] | undefined;
	let lastOutput: string[] = [];
	return {
		render(width) {
			if (width < 1) return [];
			const lines = getLines();
			if (width !== lastWidth || lines !== lastLines) {
				lastWidth = width;
				lastLines = lines;
				lastOutput = lines.flatMap((source) => {
